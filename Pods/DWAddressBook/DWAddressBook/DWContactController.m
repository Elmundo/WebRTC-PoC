...
@property(nonatomic, strong) DWMobNumberController *mobNumberVC;
@end
   ...
   PPPersonModel *people = [self.cellModelDict[self.cellModelKeysArr[indexPath.section]] objectAtIndex:indexPath.row];
    if (people.mobileArray.count > 1) {
        ...
        if (!self.showTableView) {
            ...
            _mobNumberVC = [[DWMobNumberController alloc] init];
            _mobNumberVC.view.backgroundColor = self.view.backgroundColor;
            _mobNumberVC.title = people.name;
            _mobNumberVC.backimage = self.backimage;
            _mobNumberVC.cellModelArr = people.mobileArray;
            _mobNumberVC.selectMobNumber = ^(NSString *name, NSString *mobNumber) {
                if (weakSelf.selectMobNumber) {
                    weakSelf.selectMobNumber(name, mobNumber);
                }
            };
            [self.navigationController pushViewController:_mobNumberVC animated:YES];
        }
    }else {
